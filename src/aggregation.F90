...
contains
   ...
   subroutine do(self, _ARGUMENTS_DO_)
      ...
      _LOOP_BEGIN_
         ...
         zagg1 = 12.5  * zfact * poc * poc    ! Jorn Eq 39, POC^2 shear term, value of 25.9 matches a6 in paper
         zagg2 = 169.7 * zfact * poc * goc    ! Jorn Eq 39, POC*GOC shear term, value of 4452. matches a7 in paper
         ...
         zaggdoc  = ( ( 12.0 * 0.3 * doc + 9.05 * poc ) * zfact       &   ! Jorn: Eq 36a, values of 0.369 and 102.4 match a1=0.37 and a2=102 in paper, respectively
         &            + 2.49 * xstep * poc ) * 0.3 * doc                     ! Jorn: presumably POC part of Eq 36c, but value of 2.4 does not match a4=5095 given in paper
         ! transfer of DOC to GOC : 
         ! 1st term is shear aggregation
         ! 2nd term is differential settling 
         zaggdoc2 = ( 1.94 * zfact + 1.37 * xstep ) * goc * 0.3 * doc  ! Jorn: Eq 36b? value of 3.53E3 matches a3 in paper. GOC*DOC Brownian part with scale factor 0.1 not in paper
         ! tranfer of DOC to POC due to brownian motion
         zaggdoc3 =  (127.8 * 0.3 * doc + 725.7 * poc) *xstep * 0.3 * doc                ! Jorn: Eq 36c, DOC part only, value of 114. matches a5 in paper
